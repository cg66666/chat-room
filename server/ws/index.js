 ...
 * @LastEditors: cg
 * @LastEditTime: 2024-09-02 10:15:33
 */
...
const checkInfo = async (ws, msg, wsList) => {
  ...
  if ((password && password !== roomInfo.password) || !roomInfo.userList.includes(user)) {
    ws.send(
      JSON.stringify({
        ...msg,
        type: 'tip',
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('close', async function () {
      ...
      if (res) {
        ...
        wsList.forEach((client) => {
          if (client.room === tempInfo.room && client.ws.readyState === WebSocket.OPEN) {
